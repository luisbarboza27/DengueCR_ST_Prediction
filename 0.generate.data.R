...
cantones_datos <- cantones_datos %>%
  ...
  select(Year, Cases, Canton, Month) %>%
...
etiquetas <- nomenclatura %>% select(-CTipo,-Tipo,-Latitud,-Longitud)
...
poblacion_distrital <- poblacion_distrital %>% 
  ...
  select(CCanton,Canton,Year,Poblacion,DTA) 
...
poblacion_cantonal <- poblacion_cantonal %>%
  ...
  select(Year,Month,CCanton,Poblacion=Poblacionap)
...
baseMODIS <- baseMODIS %>% 
  select(scale,DTA=site,calendar_date,value,variable)
...
baseMODIS <- baseMODIS %>%
  ...
  mutate(
      ano = year(fecha),
      Month = month(fecha)
    ) %>% select(Year=ano,Month,ValorMODIS,variable,DTA) %>% 
